...
This CLI ships a self-learning loop. When you run company-goat-pp-cli commands, the binary
...
`company-goat-pp-cli --no-learn <command>`) to disable the loop's side effects for
one invocation.
