...
// company-goat-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/company-goat/internal/learn"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/company-goat/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/company-goat/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/company-goat/internal/learn/patterns"
...
const noLearnEnvVar = "COMPANY_GOAT_NO_LEARN"
...
func learningsStateDir() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "company-goat-pp-cli")
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("company-goat-pp-cli")
}
...
~/.local/share/company-goat-pp-cli/teach.log, safe to fire-and-forget.

Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  company-goat-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  company-goat-pp-cli recall "<question>" --agent
  company-goat-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  company-goat-pp-cli learnings list --agent
  company-goat-pp-cli learnings list --query "<substring>"
  company-goat-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  company-goat-pp-cli learnings forget "<question>" --resource <id>
  company-goat-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
into a whole family.`,
		Example: `  company-goat-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  company-goat-pp-cli teach-lookup --kind country --canonical "United States" --alias USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if learnDryRunOK(flags) {
				return nil
			}
